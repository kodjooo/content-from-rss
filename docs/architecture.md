...
- `app.orchestrator` — управление основным сценарием: сбор новостей → оценка → генерация поста → подбор изображения → запись в Google Sheets; содержит логику ретраев и метрик, очистку таблицы при утреннем запуске, проверку на дубликаты по ссылке и присвоение статуса `Revised` для записей с оценкой 8–10 (иначе `Written`). Если изображение получить не удалось, строка добавляется в таблицу с пустыми полями для изображения.
