...
- `app.scoring` — обращение к OpenAI Responses API для оценки релевантности, кэширование по URL и повторные попытки. Промт построен как рубрика: главный фильтр «что читатель может сделать с этим в понедельник утром» (иначе не выше 5), повышение оценки за темы рынка труда/HR, промптов и работы с LLM, AI-агентов и AI-кодинга, понижение за инвестраунды, оценки компаний, иски и регулирование, а также за анонсы малоизвестных продуктов без применения.
- `app.post_generator` — генерация трёх версий поста (short до 600 символов для добавления CTA, average до 1000 символов без пробелов, long 1200–1500 символов, 3–4 русских хэштега), с повторным запросом при любых ошибках валидации (до трёх попыток).
  - Аудитория и персона: практики, автоматизирующие работу вайб-кодингом (ChatGPT, Claude Code, локальные модели, Python-скрипты через AI), без приписывания автору опыта no-code платформ.
  - Стилевые требования промта: стоп-лист формулировок-маркеров «нейрослопа», правила заголовка (40–70 символов, «Как …» или утверждение с цифрой, без вопроса и без шаблона «что это значит для …»), цифра в первой строке, максимум один вопрос в конце, оговорки не длиннее двух предложений, один спорный тезис, проверка на подстановку другой компании и цифры.
  - Защита от однотипности: для каждой новости по хэшу ссылки выбирается своя комбинация приёмов — тип лида, форма текста, тип финала и тон (`_LEAD_VARIANTS`, `_SHAPE_VARIANTS`, `_ENDING_VARIANTS`, `_VOICE_VARIANTS`).
  - Программные проверки (`_validate_style`): жёстко отклоняются стоп-фразы в любой версии текста, вопрос в заголовке и более одного вопроса в длинной версии; мягко (предупреждение в лог) фиксируются отсутствие цифры в первых 400 символах и длина заголовка вне 40–70 символов.
- `app.image_pipeline` — многошаговый поиск изображения: RSS → Pexels (по хэштегам, если есть) → генерация через OpenAI Images; загрузка результата на FreeImageHost. Поиск в Pexels и генерация отключены, если `ENABLE_IMAGE_GENERATION=false` (значение по умолчанию): тогда используется только картинка из самой новости.
...
- `app.orchestrator` — управление основным сценарием: сбор новостей → оценка → генерация поста → подбор изображения → запись в Google Sheets; содержит логику ретраев и метрик, очистку таблицы в первом за сутки прогоне (по часу самого раннего времени из `SCHEDULER_RUN_TIMES`), проверку на дубликаты по ссылке и присвоение статуса `Revised` для записей с оценкой 8–10 (иначе `Written`). Если изображение получить не удалось, строка добавляется в таблицу с пустыми полями для изображения.
- `app.scheduler` — планировщик запусков на основе `APScheduler`, регистрирует задачу `PipelineRunner` на времена из `SCHEDULER_RUN_TIMES` (по умолчанию один прогон в 17:50 Europe/Moscow, поддерживаются минуты и несколько времён через запятую).
...
-------------------------
1. `scheduler` инициирует запуск `PipelineRunner` в заданные времена (по умолчанию 17:50 МСК, один раз в сутки).
...
5. `orchestrator` отбирает кандидатов, предпочитая баллы 10 → 9 → 8, не более `PIPELINE_MAX_POSTS` за цикл (по умолчанию 2). Лимит низкий сознательно: два и более поста в день конкурируют за охват, поэтому таблица — короткий список кандидатов, из которого вручную публикуется один пост в день.
...
- `OPENAI_IMAGE_API_KEY` — отдельный ключ OpenAI для генерации изображений (если отличается от основного).
...
- `SCHEDULER_RUN_TIMES` — времена запусков в формате `HH:MM` через запятую (по умолчанию `17:50`). Глубина окна отбора новостей вычисляется из расписания: максимальный интервал между запусками плюс час запаса (`SchedulerConfig.lookback_hours`), для одного прогона в сутки — 25 часов.
...
- `PIPELINE_MAX_ITEMS` — ограничение числа новостей, попадающих в оценку за цикл.
- `PIPELINE_MAX_POSTS` — максимальное число постов-кандидатов, записываемых за один прогон (по умолчанию 2).
- `ENABLE_IMAGE_GENERATION` — включает поиск в Pexels и генерацию иллюстраций через OpenAI (по умолчанию false; при false картинка берётся только из RSS).
...
- Убедиться, что планировщик в контейнере регистрирует задание на время из `SCHEDULER_RUN_TIMES` (лог `Запланирован запуск пайплайна на 17:50 (Europe/Moscow)`).
