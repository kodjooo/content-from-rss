...
Этап 11. Настройка контента по данным анализа архива Сетки — выполнено
- Переписать промт генерации под аудиторию вайб-кодинга (ChatGPT/Claude Code/локальные модели), убрать заявленный опыт n8n/Make.
- Добавить стоп-лист формулировок-маркеров слопа, правила заголовка, требования к лиду с цифрой, одному вопросу в конце и спорному тезису.
- Добавить рандомизацию приёмов подачи (лид/структура/финал/тон) по хэшу ссылки, чтобы посты не были однотипными.
- Ввести программные валидаторы стиля: стоп-лист, вопрос в заголовке, число вопросов в тексте, минимальная длина 1100 символов; мягкие предупреждения в логах для цифры в лиде и длины заголовка.
- Переписать промт скоринга в рубрику с приоритетами тем (+HR/промпты/агенты, −раунды/регулирование) и фильтром практической применимости.
- Ограничить выдачу за прогон переменной `PIPELINE_MAX_POSTS` (по умолчанию 2) вместо фиксированных 3–5.
- Отключить по умолчанию поиск и генерацию иллюстраций флагом `ENABLE_IMAGE_GENERATION`.
- Обновить `.env`, `.env.example`, `docs/architecture.md`, `docs/requirements.md` и тесты.

Этап 12. Согласованное расписание генерации — выполнено
- Вынести расписание в переменную `SCHEDULER_RUN_TIMES` (формат `HH:MM`, через запятую несколько времён) вместо зашитых в код 07:00 и 19:00.
- Установить согласованное значение: один прогон в сутки в 17:50 Europe/Moscow.
- Считать окно отбора новостей от интервала между запусками плюс час запаса (`SchedulerConfig.lookback_hours`) вместо фиксированных 12 часов.
- Обновить `.env`, `.env.example`, README, `docs/architecture.md`, `docs/requirements.md` и тесты планировщика.
