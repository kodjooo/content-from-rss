...
class RelevanceScorer:
    ...
    def _build_prompt(self, item: NewsItem) -> str:
        ...
        return (
            "Оцени по шкале от 1 до 10, насколько эта новость даёт повод для практического применения, разбора ошибок "
            "или сильного инсайта для практиков, которые автоматизируют работу вайб-кодингом: ChatGPT, Claude Code, "
            "локальные модели, скрипты на Python, написанные с помощью AI.\n\n"
            "Главный фильтр: есть ли здесь то, что читатель может сделать со своим рабочим процессом в понедельник утром? "
            "Если нет — оценка не выше 5, каким бы громким ни был повод.\n\n"
            "Повышай оценку (+1…+2):\n"
            "- рынок труда, найм, HR, влияние ИИ на профессии;\n"
            "- промпты, работа с контекстом, поведение и ограничения LLM;\n"
            "- AI-агенты и AI-кодинг на практике, локальные модели, инструменты разработки;\n"
            "- разборы провалов, ошибок и реальных внедрений с деталями.\n\n"
            "Понижай оценку (−2…−3):\n"
            "- инвестиционные раунды, оценки компаний, «X привлёк $Y млрд», отчётность;\n"
            "- иски, антимонопольные расследования, регулирование и юридические сюжеты;\n"
            "- анонс малоизвестного продукта без понятного применения;\n"
            "- новость, из которой можно сделать только вывод «ИИ меняет рынок».\n\n"
