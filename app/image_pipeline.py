...
class ImageSelector:
    ...
    def _build_image_prompt(self, news: NewsItem, post: GeneratedPost) -> str:
        ...
        return (
            "Ты — ассистент для генерации промтов к изображениям.\n"
            "Твоя задача: по полученному тексту статьи или поста создать один короткий и понятный промт для генерации изображения через модель gpt image 1.\n\n"
            "Правила:\n"
            "1. Извлекай из текста ключевые образы, метафоры или сцены, которые можно визуализировать.\n"
            "2. Сохраняй стиль изображения реалистичным, допускается лёгкий символизм.\n"
            "3. Не пересказывай текст и не добавляй длинные описания — максимум два-три предложения.\n"
            "4. Изображение должно отражать основную идею или настроение текста, а не быть инфографикой.\n"
            "5. Не упоминай в промте текст статьи, автора, название или прямые цитаты.\n"
            "6. На изображении не должно быть текста.\n\n"
            "На основе данных сформируй финальный лаконичный промт.\n"
